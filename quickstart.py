...
def cleantext(raw_html):
    ...
    cleantxt = cleantxt.replace(
        "--Sent from my reMarkable paper tabletGet yours at www.remarkable.comPS: You cannot reply to this email"
        , "")
    cleantxt = cleantxt.replace("&quot;", "'")
...
def cleanhtml(raw_html):
  ...
  cleanr = re.compile(r'<[^>]+>') # works better
  #cleanr = re.compile(r'<[^<]+?>') # same
                      
  cleant = re.sub(cleanr, '', raw_html)
  return cleant
